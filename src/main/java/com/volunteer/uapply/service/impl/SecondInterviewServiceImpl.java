...
public class SecondInterviewServiceImpl implements SecondInterviewService {
    ...
    @Override
    public UniversalResponseBody<List<ApplyPO>> listUserUnSecondInterview(Integer departmentId) {
        List<ApplyPO> lists = applyMsgMapper.listApplyPOUnSecondInterview(departmentId);
        if(lists == null){
            //可能查询结果 实际为空
    ...
    @Override
    public UniversalResponseBody<List<ApplyPO>> listUserSecondedInterviewed(Integer departmentId) {
        List<ApplyPO> lists = applyMsgMapper.listApplyPOSecondedInterviewed(departmentId);
        if(lists == null){
            //可能查询结果 实际为空
            log.error("查询二面待面试人员信息，数据库查询结果为0；查询结果可能 实际为空 或 数据库查询错误！");
        }
        return new UniversalResponseBody<>(ResponseResultEnum.SUCCESS.getCode(),ResponseResultEnum.SUCCESS.getMsg(),lists);
    }
